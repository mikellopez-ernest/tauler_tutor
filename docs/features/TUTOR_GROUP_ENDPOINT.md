...
2. Use the first two parent account IDs only.
3. Fetch or resolve those parent/contact accounts through the Dinantia API.
4. If a student has more than two contacts, ignore the rest for this page.
5. If a contact slot has no data, show `-`.

### Contactes Table

The table has 7 visible columns total:
...
The table must use two header rows.

First header row:

| Header | Span |
| --- | --- |
| Student column | Covers the student name column. |
| `Contacte 1` | Covers contact 1 name, phone, and email. |
| `Contacte 2` | Covers contact 2 name, phone, and email. |
...
| Student name | `Alumne` |
| Contact 1 name | `Nom` |
| Contact 1 phone | `Telèfon` |
| Contact 1 email | `Email` |
| Contact 2 name | `Nom` |
| Contact 2 phone | `Telèfon` |
...
Allowed `field_changed` values for contact edits:
