...
function doPost(e) {
  try {
    ...
    return renderMessagePage_('No s’ha pogut continuar', 'La petició no és vàlida.', true);
  } catch (error) {
    console.error(error && error.stack ? error.stack : error);
    return renderLauncherError_(error);
  }
...
function handleParentVerification_(payload) {
  ...
  if (!isValidEmail_(email)) return renderParentEntry_('Introdueix una adreça de correu electrònic vàlida.');
...
function handleParentStudentPreSelection_(payload) {
  ...
  if (!selected) return renderMessagePage_('No s’ha pogut continuar', 'L’alumne seleccionat no correspon a aquesta adreça de correu.', true);
...
function handleStudentVerification_(payload) {
  ...
  if (!isValidEmail_(email) || !/@iernestlluch\.cat$/i.test(email)) {
    return renderStudentEntry_('Introdueix una adreça de correu electrònic del centre vàlida.');
  }
...
function handleTokenGet_(rawToken) {
  ...
  return renderMessagePage_('Enllaç no vàlid', 'Aquest enllaç de verificació no és vàlid.', true);
}
...
function handleParentStudentSelection_(payload) {
  ...
  var studentId = stringValue_(payload.student_id);
  var record = validateToken_(rawToken, { allowPendingOnly: true });
  if (record.sender !== 'parent') return renderMessagePage_('Enllaç no vàlid', 'Aquest enllaç no correspon a un accés familiar.', true);
  var metadata = parseJson_(record.metadata_json) || {};
  var child = (metadata.children || []).filter(function(item) { return String(item.id) === String(studentId); })[0];
  if (!child) return renderMessagePage_('No s’ha pogut continuar', 'L’alumne seleccionat no correspon a aquest enllaç.', true);
  return renderParentStudentOutcome_(rawToken, child, metadata.auth || null);
}
...
function renderParentStudentOutcome_(rawToken, child, existingAuth) {
  ...
  if (Number(detailed.age) >= 18) {
    return renderMessagePage_('Alumne/a major d’edat', 'Aquest formulari ha de ser emplenat pel mateix alumne/a perquè és major d’edat. Si teniu qualsevol dubte, poseu-vos en contacte amb el centre.', false);
  }
  if (auth) {
    var record = validateToken_(rawToken, { allowPendingOnly: true });
    ...
    return renderFormAccessMessage_(rawToken, formPayloadForExistingAuthorization_(auth, detailed, owner ? 'edit_owner' : 'readonly', record, rawToken), owner ? 'Formulari ja emplenat' : 'Aquest formulari ja consta com a emplenat.', owner ? 'Ja existeix una resposta per aquest alumne/a. Com que coincideix amb la persona que la va emplenar, pots revisar-la i desar canvis sobre la mateixa resposta.' : 'Podeu consultar-ne la informació en mode només lectura. Si detecteu alguna errada o voleu fer qualsevol consulta o reclamació, poseu-vos en contacte amb el centre.', owner ? 'EDITAR EL FORMULARI' : 'VEURE EL FORMULARI');
  }
...
function renderStudentOutcome_(rawToken, student, auth, isAdult) {
  ...
  var message = mode === 'student_confirm' ? 'Revisa la informació en mode només lectura i confirma la teva conformitat.' : 'La teva confirmació ja consta registrada.';
...
function handleForwardForm_(payload) {
  ...
  var session = createFormSession_(rawToken, formPayload);
  return renderOpenFormSession_(session.rawSession);
}
...
function renderSenderChoice_() {
  return htmlPage_('Accés al formulari', '<h1>Accés al formulari</h1><p>Selecciona com vols accedir al formulari.</p><div class="actions"><a class="button" href="' + escapeHtml_(LAUNCHER_CONFIG.launcherUrl + '?sender=parent') + '">Família</a><a class="button secondary" href="' + escapeHtml_(LAUNCHER_CONFIG.launcherUrl + '?sender=student') + '">Alumne/a</a></div>');
}

function renderParentEntry_(error) {
  var body = '<h1>Verificació prèvia de la identitat</h1>' +
    (error ? '<div class="alert">' + escapeHtml_(error) + '</div>' : '') +
    '<p>Benvolguda família,</p>' +
    '<p>Per tal de poder accedir al formulari d’autoritzacions, declaracions i comunicacions, és necessari verificar prèviament la identitat de la persona que l’emplenarà.</p>' +
    '<p>Aquest procés ens permet garantir que només el pare, la mare, el tutor o la tutora legal de l’alumne/a (o el mateix alumne/a, quan sigui major d’edat) pot accedir a les seves dades personals, efectuar les declaracions corresponents i signar les autoritzacions requerides.</p>' +
    '<p>Per començar, introduïu la vostra adreça de correu electrònic al camp inferior i premeu el botó <strong>Continuar</strong>.</p>' +
    '<p>Si l’adreça introduïda correspon a una persona autoritzada, rebreu en pocs instants un missatge de correu electrònic amb un enllaç personal i segur de verificació.</p>' +
    '<p>Una vegada hàgiu confirmat la vostra identitat mitjançant aquest enllaç, podreu accedir al formulari i completar-lo amb totes les garanties de seguretat.</p>' +
    '<p><strong>Important:</strong> si no rebeu el correu electrònic al cap d’uns minuts, reviseu també la carpeta de correu brossa o correu no desitjat. Si després de revisar-la continueu sense haver-lo rebut, poseu-vos en contacte amb el centre.</p>' +
    '<form method="post" action="' + escapeHtml_(LAUNCHER_CONFIG.launcherUrl) + '"><input type="hidden" name="action" value="verify_parent"><label>Correu electrònic<span>Introduïu l’adreça de correu electrònic amb la qual el centre es comunica habitualment amb la vostra família.</span><input name="email" type="email" required></label><button class="button" type="submit">Continuar</button></form>' +
    '<h2>Protecció de dades</h2><p class="small">L’adreça de correu electrònic facilitada s’utilitzarà exclusivament per verificar la vostra identitat, permetre l’accés segur al formulari i enviar-vos les comunicacions necessàries relacionades amb aquest procés. Les dades seran tractades pel centre educatiu d’acord amb la normativa vigent en matèria de protecció de dades personals i no s’utilitzaran per a finalitats diferents de les derivades de la gestió administrativa i educativa del centre.</p>';
  return htmlPage_('Verificació prèvia de la identitat', body);
}
...
function renderStudentEntry_(error) {
  ...
  var body = '<h1>Accés al formulari d’autoritzacions</h1>' +
    (error ? '<div class="alert">' + escapeHtml_(error) + '</div>' : '') +
    '<p>Benvolgut/da,</p>' +
    '<p>Aquest accés està pensat per a l’alumnat del centre. Si ets major d’edat, podràs emplenar directament el formulari d’autoritzacions, declaracions i comunicacions. Si tens 14 anys o més i la teva família ja ha emplenat el formulari, podràs revisar-lo i confirmar la teva conformitat.</p>' +
    '<p>Per aquest motiu, abans d’accedir al formulari és necessari verificar la teva identitat.</p>' +
    '<p>Per començar, introdueix la teva adreça de correu electrònic i selecciona el curs o grup al qual pertanys aquest curs escolar. Amb aquesta informació podrem localitzar el teu expedient i obrir el procés que correspongui en el teu cas.</p>' +
    '<p>Si les dades són correctes, rebràs un correu electrònic amb un enllaç personal i segur que et permetrà continuar el procés.</p>' +
    '<p><strong>Important:</strong> si no reps el correu electrònic al cap d’uns minuts, revisa també la carpeta de correu brossa o correu no desitjat. Si després de revisar-la continues sense haver-lo rebut, posa’t en contacte amb el centre.</p>' +
    '<form method="post" action="' + escapeHtml_(LAUNCHER_CONFIG.launcherUrl) + '"><input type="hidden" name="action" value="verify_student"><label>Correu electrònic<span>Introdueix l’adreça de correu electrònic que tens registrada al centre educatiu.</span><input name="email" type="email" required></label><label>Curs o grup<span>Selecciona el curs o grup al qual pertanys aquest curs escolar.</span><select name="group" required><option value="">Selecciona el teu curs o grup</option>' + options + '</select></label><button class="button" type="submit">Continuar</button></form>' +
  ...
  return htmlPage_('Accés al formulari d’autoritzacions', body);
}
...
function renderParentPreEmailStudentChoice_(email, children) {
  ...
  var body = '<h1>Selecciona l’alumne/a</h1>' +
    '<p>Hem trobat més d’un alumne/a menor d’edat associat a aquesta adreça. Selecciona per a qui vols iniciar el formulari.</p>' +
    ...
    '<button class="button" type="submit">Enviar correu de verificació</button>' +
    '</form>';
  return htmlPage_('Selecciona l’alumne/a', body);
}

function renderNotRegistered_() {
  return renderMessagePage_('Correu no registrat', 'L’adreça de correu electrònic introduïda no consta registrada a Dinantia. Utilitzeu una adreça de correu electrònic registrada al centre, o poseu-vos en contacte amb l’institut si teniu dubtes.', true);
}

function renderStudentNotRegistered_() {
  return renderMessagePage_('Dades no trobades', 'No hem pogut localitzar cap alumne/a amb aquest correu electrònic dins del grup seleccionat. Utilitza el correu i el grup registrats al centre, o posa’t en contacte amb l’institut si tens dubtes.', true);
}
...
function renderNoMinorChildren_() {
  return renderMessagePage_('Cap alumne/a menor d’edat disponible', 'No consta cap alumne/a menor de divuit anys associat a aquesta adreça. En cas d’alumnat major d’edat, el formulari l’ha d’emplenar directament l’alumne/a. Si teniu qualsevol dubte, poseu-vos en contacte amb el centre.', true);
}

function renderCheckEmail_() {
  return renderMessagePage_('Revisa el correu electrònic', 'Si l’adreça indicada correspon a una persona autoritzada, rebreu un correu electrònic amb l’enllaç de verificació.', false);
}
...
function renderFamilyMessage_(token, payload) {
  ...
  var body = '<p>Benvolguda família,</p><p>Per tal d’iniciar correctament el curs escolar i mantenir actualitzada la informació de l’alumnat, us demanem que empleneu el formulari d’autoritzacions, declaracions i comunicacions corresponent al vostre fill o filla.</p><p>En aquest formulari podreu:</p><ul><li>revisar i actualitzar les dades bàsiques;</li><li>autoritzar les diferents activitats i serveis del centre;</li><li>indicar persones autoritzades per recollir l’alumne/a;</li><li>facilitar la informació sanitària rellevant;</li><li>signar electrònicament les autoritzacions.</li></ul><p>Per accedir al formulari, feu clic al següent botó:</p><form method="post" action="' + escapeHtml_(LAUNCHER_CONFIG.launcherUrl) + '"><input type="hidden" name="action" value="forward_form"><input type="hidden" name="token" value="' + escapeHtml_(token) + '"><input type="hidden" name="form_payload_json" value="' + escapeHtml_(formPayloadJson) + '"><button class="button" type="submit">EMPLENAR EL FORMULARI</button></form><p>Temps aproximat: 10 minuts.</p><p>És important que el formulari sigui emplenat abans del dia 01/10/2026, ja que aquesta informació serà utilitzada durant tot el curs escolar.</p><p>Les dades facilitades seran tractades exclusivament per a finalitats educatives i administratives, d’acord amb la normativa vigent en matèria de protecció de dades personals.</p><p>Si teniu qualsevol incidència tècnica o dubte sobre el formulari, podeu contactar amb el centre a través del correu:</p><p><strong>' + escapeHtml_(LAUNCHER_CONFIG.supportEmail) + '</strong></p><p>Moltes gràcies per la vostra col·laboració.</p><p>Cordialment,</p><p>Equip Directiu<br>Institut Ernest Lluch<br>Cunit</p>';
...
function renderAdultStudentMessage_(token, payload) {
  ...
  var body = '<p>Benvolgut/da' + (studentName ? ' ' + escapeHtml_(studentName) : '') + ',</p>' +
    ...
    '<p>És important que el formulari sigui emplenat abans del dia 01/10/2026, ja que aquesta informació serà utilitzada durant tot el curs escolar.</p>' +
    '<p>Les dades facilitades seran tractades exclusivament per a finalitats educatives i administratives, d’acord amb la normativa vigent en matèria de protecció de dades personals.</p>' +
...
function renderAutoPostToForm_(payload) {
  ...
  var html = '<!doctype html><html lang="ca"><head><base target="_top"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Obrint el formulari</title><style>body{margin:0;background:#f6f7f9;color:#17202a;font-family:Arial,sans-serif;line-height:1.5}.fallback{display:none;width:min(680px,calc(100% - 32px));margin:32px auto;background:#fff;border:1px solid #d9e0e8;border-radius:8px;padding:30px;box-shadow:0 16px 40px rgba(21,34,50,.10)}.show-fallback .fallback{display:block}h1{font-size:26px;line-height:1.2;margin:0 0 12px}.button{display:inline-block;border:0;border-radius:6px;background:#0f766e;color:#fff;cursor:pointer;font-size:16px;font-weight:800;padding:13px 18px;text-decoration:none}.note{color:#617080}</style></head><body><form id="forwardForm" method="post" action="' + escapeHtml_(LAUNCHER_CONFIG.authFormUrl) + '">' + inputs + '<main class="fallback" id="fallback"><h1>Obrint el formulari...</h1><p class="note">Si el formulari no s’obre automàticament, prem el botó.</p><button class="button" type="submit">Obrir formulari</button></main></form><script>(function(){var form=document.getElementById("forwardForm");var fallbackTimer=setTimeout(function(){document.body.className="show-fallback";},1500);if(!form)return;try{form.submit();}catch(error){clearTimeout(fallbackTimer);document.body.className="show-fallback";}})();</script></body></html>';
...
function renderOpenFormSession_(rawSession) {
  var url = LAUNCHER_CONFIG.authFormUrl + '?form_session=' + encodeURIComponent(rawSession);
  var html = '<!doctype html><html lang="ca"><head><base target="_top"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Obrint el formulari</title><style>body{margin:0;background:#f6f7f9;color:#17202a;font-family:Arial,sans-serif;line-height:1.5}.loading-screen{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:#f6f7f9}.loading-card{display:flex;flex-direction:column;align-items:center;gap:14px;background:#fff;border:1px solid #d9e0e8;border-radius:8px;padding:26px 34px;box-shadow:0 16px 40px rgba(21,34,50,.12);font-weight:800}.loading-spinner{width:42px;height:42px;border:4px solid #d9e0e8;border-top-color:#0f766e;border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.fallback{display:none;width:min(680px,calc(100% - 32px));margin:32px auto;background:#fff;border:1px solid #d9e0e8;border-radius:8px;padding:30px;box-shadow:0 16px 40px rgba(21,34,50,.10)}.show-fallback .loading-screen{display:none}.show-fallback .fallback{display:block}h1{font-size:26px;line-height:1.2;margin:0 0 12px}.button{display:inline-block;border:0;border-radius:6px;background:#0f766e;color:#fff;cursor:pointer;font-size:16px;font-weight:800;padding:13px 18px;text-decoration:none}.note{color:#617080}</style></head><body><div class="loading-screen" role="status" aria-live="polite" aria-busy="true"><div class="loading-card"><div class="loading-spinner" aria-hidden="true"></div><div>Carregant el formulari</div></div></div><main class="fallback"><h1>Obrint el formulari...</h1><p class="note">Si el formulari no s’obre automàticament, prem el botó.</p><p><a class="button" href="' + escapeHtml_(url) + '">Obrir formulari</a></p></main><script>(function(){var target=' + JSON.stringify(url) + ';setTimeout(function(){document.body.className="show-fallback";},3500);window.location.replace(target);})();</script></body></html>';
  return HtmlService.createHtmlOutput(html)
    .setTitle('Obrint el formulari')
    .addMetaTag('viewport', 'width=device-width, initial-scale=1')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
...
function createFormSession_(rawToken, formPayload) {
  var record = validateToken_(rawToken, { allowPendingOnly: true });
  var rawSession = Utilities.getUuid() + '-' + Utilities.getUuid();
  var now = new Date();
  var expires = new Date(Math.min(
    now.getTime() + LAUNCHER_CONFIG.formSessionMinutes * 60 * 1000,
    new Date(record.expires_at).getTime()
  ));
  var metadata = parseJson_(record.metadata_json) || {};
  var safePayload = Object.assign({}, formPayload || {});
  delete safePayload.launcher_token;
  delete safePayload.form_session;
  if (record.sender === 'parent') {
    if (!safePayload.responent_nom_sencer && metadata.parent_name) safePayload.responent_nom_sencer = metadata.parent_name;
    if (!safePayload.responent_telefon && metadata.parent_phone) safePayload.responent_telefon = metadata.parent_phone;
    if (!safePayload.responsable_nom && metadata.parent_name) safePayload.responsable_nom = metadata.parent_name;
  }
  metadata.form_session = {
    session_hash: hashToken_(rawSession),
    created_at: formatDateTime_(now),
    expires_at: formatDateTime_(expires),
    payload: safePayload
  };
  var sheet = openTableSheet_('Autoritzacions', 'verification_tokens');
  var h = headerMap_(sheet);
  sheet.getRange(record._rowNumber, h.metadata_json + 1).setValue(JSON.stringify(metadata));
  return { rawSession: rawSession };
}
...
function sendVerificationEmail_(email, sender, rawToken, student) {
  ...
  var subject = sender === 'student' ? 'Enllaç de confirmació del formulari d’autoritzacions' + suffix : sender === 'student_adult' ? 'Formulari d’autoritzacions' + suffix : 'Formulari d’autoritzacions' + suffix;
  var plain = sender === 'student'
    ? 'Benvolgut/da,\n\nPer revisar el formulari d’autoritzacions' + (studentName ? ' de ' + studentName : '') + ' i confirmar la teva conformitat, obre aquest enllaç personal i segur:\n\n' + url + '\n\nAquest enllaç caduca en ' + lifetime + '.\n\nInstitut Ernest Lluch'
    : sender === 'student_adult'
    ? 'Benvolgut/da,\n\nPer tal d’iniciar correctament el curs escolar i mantenir actualitzada la teva informació, et demanem que emplenis el teu formulari d’autoritzacions, declaracions i comunicacions.\n\nEn aquest formulari podràs revisar les teves dades bàsiques, autoritzar les activitats i serveis del centre, facilitar la informació sanitària rellevant i signar electrònicament les autoritzacions necessàries per al curs escolar.\n\nPer accedir-hi, fes clic al botó següent:\n\n' + url + '\n\nAquest enllaç és personal i caduca en ' + lifetime + '.\n\nTemps aproximat: 10 minuts.\n\nSi tens qualsevol incidència tècnica o dubte sobre el formulari, pots contactar amb el centre a través del correu:\n\n' + LAUNCHER_CONFIG.supportEmail + '\n\nCordialment,\n\nEquip Directiu\nInstitut Ernest Lluch\nCunit'
    : 'Benvolgut/da,\n\nPer continuar el procés d’autoritzacions' + (studentName ? ' de ' + studentName : '') + ', obre aquest enllaç personal i segur:\n\n' + url + '\n\nAquest enllaç caduca en ' + lifetime + '.\n\nInstitut Ernest Lluch';
  var html = sender === 'student' ? studentVerificationEmailHtml_(url, student) : sender === 'student_adult' ? adultStudentVerificationEmailHtml_(url, student) : '<div style="font-family:Arial,sans-serif;line-height:1.5;color:#17202a;max-width:720px">' +
    '<p>Benvolguda família,</p>' +
    '<p>Per tal d’iniciar correctament el curs escolar i mantenir actualitzada la informació de l’alumnat, us demanem que empleneu el formulari d’autoritzacions, declaracions i comunicacions' + (studentName ? ' corresponent a <strong>' + escapeHtml_(studentName) + '</strong>' : ' corresponent al vostre fill o filla') + '.</p>' +
    '<p>En aquest formulari podreu:</p>' +
    '<ul><li>revisar i actualitzar les dades bàsiques;</li><li>autoritzar les diferents activitats i serveis del centre;</li><li>indicar persones autoritzades per recollir l’alumne/a;</li><li>facilitar la informació sanitària rellevant;</li><li>signar electrònicament les autoritzacions.</li></ul>' +
    '<p>Per accedir al formulari, feu clic al següent botó:</p>' +
    ...
    '<p>Aquest enllaç és personal i caduca en ' + lifetime + '.</p>' +
    '<p>Temps aproximat: 10 minuts.</p>' +
    '<p>És important que el formulari sigui emplenat abans del dia 01/10/2026, ja que aquesta informació serà utilitzada durant tot el curs escolar.</p>' +
    '<p>Les dades facilitades seran tractades exclusivament per a finalitats educatives i administratives, d’acord amb la normativa vigent en matèria de protecció de dades personals.</p>' +
    '<p>Si teniu qualsevol incidència tècnica o dubte sobre el formulari, podeu contactar amb el centre a través del correu:</p>' +
    '<p><strong>' + escapeHtml_(LAUNCHER_CONFIG.supportEmail) + '</strong></p>' +
...
function adultStudentVerificationEmailHtml_(url, student) {
  ...
  return '<div style="font-family:Arial,sans-serif;line-height:1.5;color:#17202a;max-width:720px">' +
    '<p>Benvolgut/da,</p>' +
    '<p>Per tal d’iniciar correctament el curs escolar i mantenir actualitzada la teva informació, et demanem que emplenis el teu formulari d’autoritzacions, declaracions i comunicacions.</p>' +
    '<p>En aquest formulari podràs revisar les teves dades bàsiques, autoritzar les activitats i serveis del centre, facilitar la informació sanitària rellevant i signar electrònicament les autoritzacions necessàries per al curs escolar.</p>' +
    '<p>Per accedir-hi, fes clic al botó següent:</p>' +
    '<p><a href="' + escapeHtml_(url) + '" style="display:inline-block;background:#0f766e;color:#fff;text-decoration:none;border-radius:6px;padding:13px 18px;font-weight:800">EMPLENAR EL FORMULARI</a></p>' +
    '<p>Aquest enllaç és personal i caduca en 24 hores.</p>' +
...
function studentVerificationEmailHtml_(url, student) {
  ...
  return '<div style="font-family:Arial,sans-serif;line-height:1.5;color:#17202a;max-width:720px">' +
    '<p>Benvolgut/da,</p>' +
    '<p>Algunes autoritzacions i declaracions del curs' + (studentName ? ' de <strong>' + escapeHtml_(studentName) + '</strong>' : '') + ' requereixen també la conformitat de l’alumnat.</p>' +
    '<p>Per revisar el formulari ja emplenat i confirmar la teva conformitat, fes clic al botó següent:</p>' +
